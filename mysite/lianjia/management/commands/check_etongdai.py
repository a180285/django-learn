...
def buildRequests():
  url = 'http://app.etongdai.com/investments/indexJson?type=1&scope=&repay=60&repayType=&orderArgs=3&ascDesc=asc&pageId=1&init_invest=1'
  datas = [
    ''
...
class Command(BaseCommand):
  ...
  def run(self):
    ...
    for req in reqs:
      ...
      html = response.read()
      parsed_json = simplejson.loads(html)
      biao_list = parsed_json['body']['pageMdl']['list']
      for biao in biao_list:
        remainMoneyYuan = biao['remainMoneyYuan']
        yearRate = biao['yearRate'] * 100
        eTitle = biao['title']
        if yearRate > 9 and remainMoneyYuan > 30000:
          self.send_notify(eTitle, eTitle)
